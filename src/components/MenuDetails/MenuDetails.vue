...
const menuItems = ref(null);  // Holds current item details
const user = inject('user'); // Injected user object
...
const singleData = async (id) => {
  try {
    loading.value = true;
    const response = await axios.get(`https://restaurant-server-xi.vercel.app/newMenu/${id}`);
    menuItems.value = response.data;  // Update current item data
  } catch (error) {
    console.error(error);
...
const addToCart = async () => {
  ...
  if (!menuItems.value) {
    toast.error('No menu item available to add to cart.');
    return;
  }
  ...
  <div class="max-w-7xl mx-auto">
    <div class="flex flex-col lg:flex-row gap-6 justify-between items-start py-6 px-3 lg:px-0">
      <div class="w-full lg:w-2/3">
        ...
        <div v-else>
          <div v-if="menuItems && menuItems.image">
            <div class="py-3">
              <img class="w-full h-[28rem] object-cover rounded-2xl shadow-lg" :src="menuItems.image" alt="Menu Image">
            </div>
